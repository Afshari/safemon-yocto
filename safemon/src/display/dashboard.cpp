...
namespace Safemon {
    ...
    static const glm::vec4 COL_TEXT_BRIGHT = { 0.92f, 0.96f, 1.0f,  1.0f };
    static const glm::vec4 COL_TEXT_MID    = { 0.49f, 0.59f, 0.72f, 1.0f };
    static const glm::vec4 COL_TEXT_DIM    = { 0.27f, 0.35f, 0.48f, 1.0f };
    ...
    void Dashboard::Init(TextRenderer* textRenderer, float design_scale)
    {
        ...
        m_text->AddSize("eyebrow", 11.0f * design_scale);
        m_text->AddSize("title",   19.0f * design_scale);
        m_text->AddSize("body",    14.0f * design_scale);
        m_text->AddSize("label",   10.0f * design_scale);
        m_text->AddSize("code",    18.0f * design_scale);
    }
    ...
    void Dashboard::DrawHeaderText(const DashboardState& state)
    {
        ...
        Text(24, 38, "STATUS PANEL", COL_TEXT_BRIGHT, "title");
    ...
    void Dashboard::DrawConnectionText(const DashboardState& state)
    {
        ...
        Text(24 + 40, CONN_Y + 12, "CAN INTERFACE", COL_TEXT_MID, "label");
        Text(24 + 40, CONN_Y + 27, state.can_ok ? "LINKED" : "ERROR",
            state.can_ok ? COL_GREEN : COL_RED, "body");

        float x2 = 24 + w + gap;
        Text(x2 + 40, CONN_Y + 12, "REDIS STORE", COL_TEXT_MID, "label");
        Text(x2 + 40, CONN_Y + 27, state.redis_ok ? "CONNECTED" : "ERROR",
    ...
    void Dashboard::DrawFaultText(const DashboardState& state)
    {
        ...
        Text(ring_cx - 4, ring_cy + 7, "!", COL_RED, "code");

        Text(x + 90, FAULT_Y + 24, "ACTIVE FAULT", COL_TEXT_MID, "label");
        Text(x + 90, FAULT_Y + 44, state.fault_code.c_str(), { 1.0f, 0.84f, 0.85f, 1.0f }, "code");
    ...
    void Dashboard::DrawTelemetryText(const DashboardState& state)
    {
        ...
        Text(x1 + 14, TELEM_Y + 12, "LAST FRAME", COL_TEXT_MID, "label");
        Text(x1 + 14, TELEM_Y + 28, state.last_frame.c_str(), COL_CYAN_SOFT, "body");
        Text(x1 + 14, TELEM_Y + 46, state.last_frame_time.c_str(), COL_TEXT_DIM, "label");

        Text(x2 + 14, TELEM_Y + 12, "FRAMES IN QUEUE", COL_TEXT_MID, "label");
        Text(x2 + 14, TELEM_Y + 28, std::to_string(state.frame_count).c_str(), COL_TEXT_BRIGHT, "body");

        Text(x3 + 14, TELEM_Y + 12, "UPTIME", COL_TEXT_MID, "label");
        Text(x3 + 14, TELEM_Y + 28, state.uptime.c_str(), COL_TEXT_BRIGHT, "body");
        Text(x3 + 14, TELEM_Y + 46, "since last reboot", COL_TEXT_DIM, "label");
    }

    void Dashboard::DrawFooterText()
    {
        Text(24, SCREEN_H - 24, "KERNEL 6.12 PREEMPT  -  SAFEMON v0.2", COL_TEXT_DIM, "label");
        Text(SCREEN_W - 220, SCREEN_H - 24, "RPI4  -  BUILD 2026.06.20", COL_TEXT_DIM, "label");
    }

    // ---------------- scale-aware wrappers ----------------
