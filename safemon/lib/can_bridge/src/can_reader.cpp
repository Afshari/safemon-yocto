...
bool CanReader::open()
{
    ...
    if (socket_fd_ < 0) {
        std::cerr << "[can_reader] Error creating CAN socket\n";
        return false;
    }
    ...
    if (ioctl(socket_fd_, SIOCGIFINDEX, &ifr) < 0) {
        std::cerr << "[can_reader] Error getting interface index\n";
        return false;
    }
    ...
    if (bind(socket_fd_, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        std::cerr << "[can_reader] Error binding CAN socket\n";
        return false;
    }
    ...
    std::cout << "[can_reader] Socket opened on " << interface_ << "\n";
    return true;
}
...
bool CanReader::read(CanFrame& frame)
{
    ...
    if (nbytes < 0) {
        ...
        std::cerr << "[can_reader] Error reading CAN frame\n";
        return false;
    }
